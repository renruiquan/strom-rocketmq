...
public class RocketMqBolt implements IRichBolt {
    ...
    public void prepare(Map<String, Object> topoConf, TopologyContext context, OutputCollector collector) {
        ...
        String producerGroup = this.properties.getProperty(SpoutConfig.PRODUCER_GROUP);

        if (StringUtils.isNotEmpty(accessKey) && StringUtils.isNotEmpty(accessSecret)) {
            producer = new DefaultMQProducer(producerGroup, new AclClientRPCHook(new SessionCredentials(accessKey, accessSecret)));
            // 自建服务=AccessChannel.LOCAL，云MQ=AccessChannel.CLOUD)
            producer.setAccessChannel(AccessChannel.CLOUD);
        } else {
            producer = new DefaultMQProducer("GID_TEST_SDK");
            // 自建服务=AccessChannel.LOCAL，云MQ=AccessChannel.CLOUD)
