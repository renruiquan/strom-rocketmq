...
public class RocketMqSpout implements IRichSpout {
    ...
    public void open(Map<String, Object> conf, TopologyContext context, SpoutOutputCollector collector) {
        ...
        String accessSecret = this.properties.getProperty("AccessSecret");
        String consumerGroup = this.properties.getProperty(SpoutConfig.CONSUMER_GROUP);

        //兼容阿里云Rocket和本地自建的RocketMQ
        if (StringUtils.isNotEmpty(accessKey) && StringUtils.isNotEmpty(accessSecret)) {
            RPCHook rpcHook = new AclClientRPCHook(new SessionCredentials(accessKey, accessSecret));
            consumer = new DefaultMQPushConsumer(consumerGroup, rpcHook, new AllocateMessageQueueAveragely(), true, null);
